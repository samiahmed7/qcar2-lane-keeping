...
class QCar2PathMPC(Node):
    ...
    def __init__(self):
        ...
        # a smooth speed ramp that starts well before its own hard
        # stop/overtake-decision thresholds, so QCar2 settles into a
        # steady gap behind a slower vehicle instead of closing at full
        # speed and hard-braking at the threshold, over and over. -1.0
        # means "no cap" (nothing ahead, or far enough that it doesn't
        # matter). Only ever LOWERS target_v (min()), so a stale/absent
        # value is safe by construction.
        self.lidar_speed_cap = -1.0
        # Same idea, sourced from the V2V along-path gap instead of raw
        # LiDAR distance -- see v2v_receiver_node.compute_follow_speed_cap.
        # Kept as a separate, independently-clamped value: the two sources
        # can fail independently (V2V link vs. LiDAR obstruction), and
        # min()'ing both in is strictly safer than picking one.
        self.v2v_follow_speed_cap = -1.0
        # Filtered separately from the raw value above (found 2026-08-27):
        # the raw cap only changes when a V2V UDP packet arrives (~10 Hz),
        # stepping discretely between packets while path_mpc's own loop
        # runs faster (12.5 Hz) -- so target_v jumped visibly on nearly
        # every packet with no smoothing, on top of the leader-speed term
        # itself carrying real noise (it's differentiated from ROSbot3's
        # AMCL position, EMA-smoothed at the source but not eliminated).
        # Same EMA pattern as avoidance_offset_filtered/track_error_filtered
        # below, just missed when the cap was first added. -1.0 (no
        # restriction) is a sentinel, not a value on the same scale as a
        # real cap -- blending across that boundary would be meaningless,
        # so it bypasses the filter both ways: snaps to -1.0 immediately
        # when the restriction lifts (relaxing early is safe), and starts
        # the filter fresh from the first real value rather than blending
        ...
        # --- localization jump detection -------------------------------
        # localization_stable() only checks the pose isn't CHANGING, not
        # that it's CORRECT. After an e-stop + localization relaunch the
        # car is stationary, so a stationary-but-WRONG pose (cartographer
        # has not yet globally relocalised into the loaded pbstream --
        # global_localization_min_score 0.75) sails straight through that
        # gate and latches localization_ready True. When cartographer then
        # finds its match the pose SNAPS discontinuously, and nothing
        # re-gates it: closest_idx can't follow (windowed + forward-only),
        # startup_v_max has already burned its 30 steps, and the
        # track_error EMA is still low -- so the car runs at full v_max
        # toward a reference unrelated to where it actually is.
        #
        # At 0.08 s/cycle and v_max 0.75 m/s, real motion is <= ~0.06 m and
        # a few deg per cycle. These thresholds are well clear of that, so
        # only a genuine relocalisation snap trips them.
        #
        # BOTH must trip together (see localization_jumped below) -- found
        # 2026-08-27 that yaw-alone at moderate values fires on legitimate
        # scan-matcher yaw refinements mid-curve (no motion prior exists,
        # see qcar2_2d_localization.lua notes), NOT relocalisation snaps.
        # Confirmed teleports always moved BOTH together (4.44m/166deg,
        # 3.34m/161deg); the false trigger that caused this fix moved
        # 0.021m/38deg -- position barely changed. Treating that as a jump
        # forced a disruptive full-stop + global re-search mid-curve, which
        # is what actually knocked the car off track, not the correction
        # itself.
        self.jump_pos_threshold = 0.30            # m in one cycle
        self.jump_yaw_threshold = math.radians(20.0)
        self.force_global_research = False
        self.jump_count = 0

        # Persist the last pose we believed was good, so that after an
        # e-stop + localization relaunch the stack can SEED cartographer with
        # it instead of making it globally relocalise from nothing. The car
        # cannot move while it is stopped, so the pose at shutdown is still
        # true at relaunch. This is what lets the tightened
        # global_localization_min_score be safe: recovery no longer depends
        # on the global search we just suppressed.
        self.last_pose_file = "/home/nvidia/ros2_ws_sami/last_known_pose.txt"
        self.pose_save_every = 10                 # ~0.8 s at 0.08 s/cycle
        self.pose_save_counter = 0

        self.tracking_log_path = "/home/nvidia/ros2_ws_sami/mpc_tracking_log.csv"
        ...
        self.create_subscription(Float32, "/lidar_speed_cap", self.lidar_speed_cap_callback, 10)
        self.create_subscription(Float32, "/v2v/follow_speed_cap", self.v2v_follow_speed_cap_callback, 10)
    ...
    def lidar_speed_cap_callback(self, msg):
        self.lidar_speed_cap = float(msg.data)
    ...
    def update_v2v_follow_cap_filter(self):
        """Smooth the raw V2V follow-speed cap before it ever touches
        target_v. See __init__ for why -- kept as its own step, called
        once per control cycle, rather than filtering in the subscription
        ...
        if raw < 0.0:
            ...
            self.v2v_follow_speed_cap_filtered = raw
        else:
            self.v2v_follow_speed_cap_filtered = (
                (1.0 - self.v2v_follow_cap_alpha) * self.v2v_follow_speed_cap_filtered
                + self.v2v_follow_cap_alpha * raw
            )

    def save_last_pose(self, pose):
        """Atomically persist the current pose for use as a relaunch seed.
        Written via tmp+rename so the seeder can never read a half-written
        line, and best-effort -- a failure here must never stop driving."""
        self.pose_save_counter += 1
        if self.pose_save_counter < self.pose_save_every:
            return
        self.pose_save_counter = 0

        try:
            tmp = self.last_pose_file + ".tmp"
            with open(tmp, "w") as fh:
                fh.write(f"{pose[0]:.6f} {pose[1]:.6f} {pose[2]:.6f}\n")
            os.replace(tmp, self.last_pose_file)
        except OSError as e:
            self.get_logger().warn(
                f"could not save last pose: {e}",
                throttle_duration_sec=30.0,
            )

    def localization_jumped(self, pose):
        """True if the pose moved further in one cycle than the car
        physically can -- i.e. cartographer relocalised and snapped.

        Owns prev_pose while localization_ready is True; localization_stable()
        owns it while it is False. The two are mutually exclusive per cycle,
        so exactly one of them advances prev_pose every cycle -- if neither
        did, prev_pose would go stale and fake a jump."""
        if self.prev_pose is None:
            self.prev_pose = pose.copy()
            return False

        dpos = math.hypot(
            pose[0] - self.prev_pose[0],
        ...
        if dpos > self.jump_pos_threshold and dyaw > self.jump_yaw_threshold:
            self.jump_count += 1
            self.get_logger().warn(
                f"Localization JUMP #{self.jump_count}: "
                f"dpos={dpos:.3f} m, dyaw={math.degrees(dyaw):.1f} deg. "
                f"Stopping, re-arming localization gate and re-searching path."
            )
            return True
    ...
    def _control_loop(self):
        ...
        # A relocalisation snap invalidates everything downstream that is
        # anchored to the old pose, so re-arm the whole gate rather than
        # driving on through the discontinuity.
        if self.localization_ready and self.localization_jumped(pose):
            self.localization_ready = False
            self.localization_counter = 0
            self.startup_counter = 0
            self.force_global_research = True
            self.stop()
            self.reset_mpc_memory()
            return
        ...
        self.closest_idx = PathUtils.closest_point(
            ...
            self.trajectory,
            self.closest_idx,
            global_search=self.force_global_research,
        )

        if self.force_global_research:
            self.force_global_research = False
            self.get_logger().warn(
                f"Path re-acquired after jump: closest_idx={self.closest_idx}"
            )

        # Only reached with localization_ready True and no jump this cycle,
        # so this is a pose we actually trust -- exactly what we want to seed
        # a relaunch with.
        self.save_last_pose(pose)
        ...
        self.update_v2v_follow_cap_filter()
        ...
        # Re-apply the speed caps AFTER the state machine. apply_state_machine_
